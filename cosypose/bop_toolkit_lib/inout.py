        ...
        with open(path, "r") as f: # pylint: disable=unspecified-encoding
            ...
            for line in f:
                ...
                if line_id == 1 and header in line:
                    ...
                    if len(elems) != 7:
                        raise ValueError(
                            f"A line does not have 7 comma-sep. elements: {line}"
                        )
    ...
    for pt_id in range(n_pts):
        ...
        if is_binary:
            for prop in pt_props:
                data_format = formats[prop[1]]
                read_data = f.read(data_format[1])
                val = struct.unpack(data_format[0], read_data)[0]
    ...
    for face_id in range(n_faces):
        ...
        if is_binary:
            for prop in face_props:
                data_format = formats[prop[1]]
                val = struct.unpack(data_format[0], f.read(data_format[1]))[0]
                if prop[0] == "n_corners":
