...
for result_filename in p["result_filenames"]:
    ...
    dataset = str(result_name.split("_")[1].split("-")[0]) # pylint: disable=invalid-name
    ...
    for error in p["errors"]:
        ...
        for error_sign, error_dir_path in error_dir_paths.items():
            for correct_th in error["correct_th"]:
                ...
                score_sign = misc.get_score_signature(correct_th, p["visib_gt_min"]) # pylint: disable=invalid-name

                scores_filename = f"scores_{score_sign}.json" # pylint: disable=invalid-name
    ...
    final_scores = {}
    for error in p["errors"]:
        final_scores[f"bop19_average_recall_{error["type"]}"] = average_recalls[
            error["type"]
        ]
