...
def get_symmetry_transformations(model_info, max_sym_disc_step):
    ...
    if "symmetries_discrete" in model_info:
        for sym in model_info["symmetries_discrete"]:
            ...
            R = sym_4x4[:3, :3]  # pylint: disable=invalid-name
    ...
    if "symmetries_continuous" in model_info:
        for sym in model_info["symmetries_continuous"]:
            ...
            for i in range(1, discrete_steps_count):
                R = transform.rotation_matrix(  # pylint: disable=invalid-name
                    i * discrete_step, axis
                )[:3, :3]
    ...
    for tran_disc in trans_disc:
        if trans_cont:
            for tran_cont in trans_cont:
                R = tran_cont["R"].dot(tran_disc["R"])  # pylint: disable=invalid-name
...
def project_pts(pts, K, R, t):  # pylint: disable=invalid-name
    """Projects 3D points.
...
class Precomputer(object):
    ...
    @staticmethod
    def precompute_lazy(depth_im, K):  # pylint: disable=invalid-name
...
def depth_im_to_dist_im_fast(depth_im, K):  # pylint: disable=invalid-name
    ...
    pre_Xs, pre_Ys = Precomputer.precompute_lazy(  # pylint: disable=invalid-name
        depth_im, K
    )
...
def depth_im_to_dist_im(depth_im, K):  # pylint: disable=invalid-name
    """Converts a depth image to a distance image.
    :param depth_im: hxw ndarray with the input depth image, where depth_im[y, x]
      is the Z coordinate of the 3D point [X, Y, Z] that projects to pixel [x, y],
      or 0 if there is no such 3D point (this is a typical output of the
    ...
    Xs = np.multiply(xs - K[0, 2], depth_im) * (  # pylint: disable=invalid-name
        1.0 / K[0, 0]
    )
    Ys = np.multiply(ys - K[1, 2], depth_im) * (  # pylint: disable=invalid-name
        1.0 / K[1, 1]
    )
...
def iou(bb_a, bb_b):
    ...
    if w_inter > 0 and h_inter > 0:
        ...
        iou_value = area_inter / float(area_a + area_b - area_inter)
    else:
        iou_value = 0.0
...
def transform_pts_Rt(pts, R, t):  # pylint: disable=invalid-name
