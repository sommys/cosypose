...
class PoseErrorMeter(Meter):
    ...
    def compute_errors(self, TXO_pred, TXO_gt, labels):
        ...
        if self.error_type.upper() == "ADD":
            ...
            if len(ids_nosym) > 0:
                dists[ids_nosym] = dists_add(
                    TXO_pred[ids_nosym], TXO_gt[ids_nosym], points[ids_nosym]
                )
            if len(ids_sym) > 0:
                dists[ids_sym] = dists_add_symmetric(
                    TXO_pred[ids_sym], TXO_gt[ids_sym], points[ids_sym]
                )
        else:
        ...
        errors["TCO_norm"] = torch.norm(
            TXO_pred[:, :3, -1] - TXO_gt[:, :3, -1], dim=-1, p=2
        )
        return errors
    ...
    def add(self, pred_data, gt_data):
        ...
        pred_data.infos = add_inst_num(
            pred_data.infos, key="pred_inst_id", group_keys=group_keys
        )
        gt_data.infos = add_inst_num(
            gt_data.infos, key="gt_inst_id", group_keys=group_keys
        )
        ...
        errors = self.compute_errors_batch(
            cand_TXO_pred, cand_TXO_gt, cand_infos["label"].values
        )
        ...
        add01d = errors_norm < 0.1 * matches["obj_diameter"]
        matches["0.1d"] = "match_id", add01d.data
