from cosypose.utils.tqdm import patch_tqdm
...
from cosypose.evaluation.pred_runner.multiview_predictions import (
...
def load_posecnn_results():
    ...
    for scene_view_str, result in results.items():
        ...
        for n in range(n_dets):
            obj_id = result["rois"][:, 1].astype(np.int32)[n]
            ...
            meters.update(
                {
                    f"{error_type}_ntop=BOP_matching=BOP": PoseErrorMeter(  # For ADD-S<0.1d
                        error_type=error_type, match_threshold=0.1, **base_kwargs
                    ),
...
def main():
    ...
    predictor, mesh_db = load_models(
        coarse_run_id, refiner_run_id, n_workers=n_plotters, object_set=object_set
    )
    ...
    if skip_predictions:
        ...
        pred_kwargs = {
            "pix2pose_detections": dict(
                detections=pix2pose_detections, **base_pred_kwargs
            ),
        }
    elif "ycbv" in ds_name:
    ...
    if get_rank() == 0:
        ...
        results = format_results(
            all_predictions, eval_metrics, eval_dfs, print_metrics=False
        )
