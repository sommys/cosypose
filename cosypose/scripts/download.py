...
BOP_SRC = "https://huggingface.co/datasets/bop-benchmark/{}/resolve/main/"
BOP_DATASETS = {
    "ycbv": {"splits": ["train_real", "train_synt", "test_all"]},
    "tless": {
    ...
    "tudl": {"splits": ["test_all", "train_real"]},
}
...
def main():
    ...
    parser.add_argument(
        "--bop_extra_files", default="", type=str, choices=["ycbv", "tless"]
    )
    ...
    if args.bop_extra_files:
        if args.bop_extra_files == "tless":
            ...
            gdrive_download(
                f"bop_datasets/tless/all_target_tless.json", BOP_DS_DIR / "tless"
            )
            ...
            gdrive_download(
                f"bop_datasets/ycbv/ycbv_friendly_names.txt", BOP_DS_DIR / "ycbv"
            )
    ...
    if args.ycbv_compat_models:
        ...
        gdrive_download(
            f"bop_datasets/ycbv/models_bop-compat_eval", BOP_DS_DIR / "ycbv"
        )
    ...
    if args.bop_result_id:
        ...
        gdrive_download(
            f"bop_predictions/{csv_name}", LOCAL_DATA_DIR / "bop_predictions"
        )
        gdrive_download(
    ...
    if args.example_scenario:
        gdrive_download(
            f"custom_scenarios/example/candidates.csv",
            LOCAL_DATA_DIR / "custom_scenarios/example",
        )
        gdrive_download(
            f"custom_scenarios/example/scene_camera.json",
            LOCAL_DATA_DIR / "custom_scenarios/example",
        )

    if args.all_bop20_models:
        from cosypose.bop_config import (
            PBR_DETECTORS,
            PBR_COARSE,
            PBR_REFINER,
            SYNT_REAL_DETECTORS,
...
def download_bop_original(ds_name, download_pbr):
    ...
    src = BOP_SRC.format(ds_name)
    wget_download_and_extract(src + filename, BOP_DS_DIR)

    suffixes = ["models"] + BOP_DATASETS[ds_name]["splits"]
    if download_pbr:
        suffixes += ["train_pbr"]
    for suffix in suffixes:
        wget_download_and_extract(src + f"{ds_name}_{suffix}.zip", BOP_DS_DIR / ds_name)
